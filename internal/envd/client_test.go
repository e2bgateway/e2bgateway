...
func TestDoConnectRPC_Success(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify headers (Connect unary uses application/json, not connect+json)
		...
		// Decode plain JSON request (Connect unary format)
		...
		// Send plain JSON response (Connect unary format)
		writeJSONResponse(w, map[string]string{"result": "success"})
	}))
	defer server.Close()
